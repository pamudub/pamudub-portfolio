...
const REVEAL_CONTENT = [
  ...
  {
    upper: profile.creativeTools.map(c => `${c.category}: ${c.tools.join(', ')}`),
...
const ITEMS = [
  ...
  {
    id: "tech", label: "CREATIVE TOOLS", handle: "Skills", href: profile.links[0].href, icon: "⚙️", barIcon: icon2, bars: profile.creativeTools.length, newBars: [0, 1], counts: profile.creativeTools.map(c => c.category),
    links: profile.creativeTools.map(c => c.tools.join(', ')),
    stats: [
      { tag: "CATS", value: profile.creativeTools.length, color: "#e8c100" },
      { tag: "TOOLS", value: profile.creativeTools.reduce((n, c) => n + c.tools.length, 0), color: "#4a8fff" },
    ],
  },
  {
